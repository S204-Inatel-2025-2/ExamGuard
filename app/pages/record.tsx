...
export default function Record() {
  ...
  useEffect(() => {
    ...
    intervalId = setInterval(() => {
        if (record && (record.status === "PROCESSING" || record.status === "PENDING")) {
            // Polling silencioso
            api.get(`/api/videos/${id}`).then(res => {
                setRecord(res.data);
            }).catch(() => {});
        }
    }, 5000);
            ...
            <Card className="h-full max-h-[800px] flex flex-col">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <p className="text-sm text-muted-foreground">Clique para ver o momento exato</p>                </CardHeader>
